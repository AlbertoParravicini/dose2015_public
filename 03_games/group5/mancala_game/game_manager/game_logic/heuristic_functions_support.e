...
feature
	...
	starting_variance: REAL_64 = 0.005
	...
	generate_gaussian_weights (a_weights:  ARRAYED_LIST [TUPLE [weight: REAL_64; variance: REAL_64]]): ARRAYED_LIST [TUPLE [weight: REAL_64; variance: REAL_64]]
		...
		do
			...
			loop
				...
				a_weights.item.weight := update_gaussian_value (box_muller_result.z1, log(a_weights.item.weight), log(a_weights.item.variance))
				a_weights.item.variance := log(a_weights.item.variance)

				a_weights.forth
				a_weights.item.weight := update_gaussian_value (box_muller_result.z2, log(a_weights.item.weight), log(a_weights.item.variance))
				a_weights.item.variance := log(a_weights.item.variance)
	...
	log_normal_weights (a_weights:  ARRAYED_LIST [TUPLE [weight: REAL_64; variance: REAL_64]]): ARRAYED_LIST [TUPLE [weight: REAL_64; variance: REAL_64]]
		...
		do
			...
			loop
				...
				a_weights.item.variance := exp (a_weights.item.variance)
				a_weights.forth
			end
...
--					bred_variance := exp((breeding_factor * better_variance + (1.0 - breeding_factor) * worse_variance) * (mean_diff / covariance))
--				else
--					bred_variance := exp(breeding_factor * better_variance + (1.0 - breeding_factor) * worse_variance)
--				end
